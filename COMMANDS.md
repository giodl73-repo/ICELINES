...
artifacts. Pass `-ExpectedSourceRevision <git-sha> -Smoke` after downloading an
artifact to bind the package manifest to the expected source and execute the
packaged binary's `--version` command before accepting it.
