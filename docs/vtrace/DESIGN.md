...
| DES-016 | Keep IceLines Signals as a descriptor/evidence-backed core module separate from stable `StatId`; selected surfaces must consume the evidence-preserving ViewModel and pass copy review before promotion. | REQ-SIGNAL-001; REQ-DATA-001; REQ-STAT-001 | Composite metrics need input coverage, polarity, limitations, and missing-data behavior before they become user-facing stats. | Add permanent stat catalog entries first or let renderers compute composites ad hoc. | `design/specs/icelines-signals.md`; IF-SIGNAL-001 |
...
| Signal metrics | `icelines-core/src/signal_metrics.rs` | Define descriptive derived metric descriptors, evidence tiers, formulas, and missing-input behavior. | No renderer-local computation, no zero-fill for missing inputs, and no predictive or deployment claims. |
