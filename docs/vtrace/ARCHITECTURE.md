...
| ADR-VT-006 | Treat the major analytics cache as the canonical future evidence layer for hockey decision surfaces. | accepted partial | Initial core cache envelope, strict store/read, and internal consumer ViewModel evidence exists; coach-facing screens/reports still cannot claim cache-backed implementation until their shipped-surface evidence exists. |
...
| Major analytics cache is partially implemented but not shipped as a user-facing product surface. | Future product copy could imply dashboards/reports are already cache-backed. | Keep shipped-surface claims pending and require WP-009 consumer/copy evidence before production dashboard/report/card claims. |
