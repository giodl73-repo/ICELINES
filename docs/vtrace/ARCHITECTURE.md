...
| Major analytics cache posture | The initial core schema/consumer contract exists, but no production major analytics cache or downstream surface is claimed. | A versioned cache builder/read model supplies canonical analytics, provenance, freshness/staleness, quality/completeness, invalidation keys, and consumer contracts for future hockey decision surfaces. | REQ-CACHE-001..004 |
...
| ADR-VT-006 | Treat the major analytics cache as the canonical future evidence layer for hockey decision surfaces. | accepted partial | Initial core cache envelope evidence exists; coach-facing screens/reports still cannot claim cache-backed implementation until their WP-009 consumer evidence exists. |
