...
| WP-009 | Build the major analytics cache foundation. | REQ-CACHE-001; REQ-CACHE-002; REQ-CACHE-003; REQ-CACHE-004; REQ-CODE-001 | analytics cache records/envelopes, cache store/read path, downstream consumer ViewModels, future dashboard/report surfaces | CHG-072 target-spec baseline accepted; storage, schema compatibility, metric family, and first consumer fixture selected. | Versioned schema, no-live read behavior, invalidation/degraded-state behavior, downstream consumer preservation, first named-cache Web report evidence, first coach dashboard route evidence, and first opponent scout route evidence recorded. | L0: schema/contract fixtures; L1: store/source-state fixtures; L2: consumer preservation and Web report fixtures | partial; pulses 02-07 passed selected core schema/envelope, strict JSON store/read/invalidation, internal consumer ViewModel, first product-facing named-cache Web report, first coach dashboard route, and first opponent scout route evidence; broader hockey product surfaces remain pending |
...
| L2 | yes | At least one consumer demo or snapshot showing cache-backed decision-support envelope and non-claim disclosure. | partial; pulses 02-07 passed in-core, store-backed coach-dashboard envelope, internal dashboard-style ViewModel proofs, first named-cache Web report, first coach dashboard route, and first opponent scout route evidence; broader card/line/goalie/practice/postgame surfaces are not claimed |
...
report, first coach dashboard route, and first opponent scout route exist in
`icelines-web::handlers::analytics_cache_report`. Downstream card/line/goalie/
practice/postgame surfaces and broader product-copy reviews remain pending.
...
the first coach dashboard route. Implementation closure requires broader
shipped-surface consumer and closeout reviews.
