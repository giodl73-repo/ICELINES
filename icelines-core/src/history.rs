...
impl SeasonLine {
    pub fn new(season: &str, team: &str, gp: u32, goals: u32, assists: u32) -> Self {
        let ppg = if gp > 0 {
            (goals + assists) as f32 / gp as f32
        } else {
            0.0
        };
        let goals_per_82 = if gp > 0 {
            goals as f32 / gp as f32 * 82.0
        } else {
            0.0
        };
        Self {
...
impl CareerSummary {
    pub fn from_seasons(
        ...
        let (peak_ppg, peak_season) = lines
            .iter()
            .filter(|l| l.gp >= 10)
            .max_by(|a, b| {
                a.ppg
...
mod tests {
    ...
    fn l0_career_ppg_weighted_by_gp() {
        ...
        assert!(
            (summary.career_ppg - 170.0 / 122.0).abs() < 0.001,
            "expected {:.3}, got {:.3}",
            170.0 / 122.0,
            summary.career_ppg
        );
