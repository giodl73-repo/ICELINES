...
async fn augment_career_history_for(display_name: &str, normalized: &str) {
    // Delegate to the shared helper now in icelines-fetch (Foster +18+).
    // CLI passes the live-feeds gate from this crate's config so the
    // user's `--no-live` / `ICELINES_NO_LIVE` choice still wins.
    icelines_fetch::career_landing::augment_career_history_for_player(
        display_name,
        normalized,
        crate::config::live_feeds_enabled(),
    )
    .await;
}
