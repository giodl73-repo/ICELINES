...
pub async fn run(
    ...
    let view = PlayerStreaksView::from_game_lines(context, pid, player_name, &lines);
    ...
    if view.games_loaded == 0 {
        let message = format!(
            "No cached boxscore game lines found for {player} ({pid}). Run `icelines fetch boxscore --date YYYY-MM-DD` to populate streak inputs."
        );
        match out {
