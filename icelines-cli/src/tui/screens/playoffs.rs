...
fn render_bracket(f: &mut Frame, area: Rect, app: &App, b: &PlayoffBracket) {
    ...
    if round.series.is_empty() {
        ...
        let active_series = app.playoffs_series.min(round.series.len().saturating_sub(1));
...
fn format_series_row(s: &PlayoffSeries) -> String {
    ...
    format!(
        "    ({:<3}) {:<3}  vs  ({:<3}) {:<3}    {}",
        top_rank, s.top_seed_abbrev,
        bot_rank, s.bottom_seed_abbrev,
        summary,
    )
}
...
pub fn render_series_detail(f: &mut Frame, app: &App, area: Rect, letter: &str) {
    ...
    let year = match playoff_year_for_season(&app.active_season) {
        Some(y) => y,
        None    => { render_message(f, inner, "No playoff data for this season."); return; }
    };
    ...
    let series = match state {
        ...
        PlayoffsState::Error(e) => { render_error(f, inner, &e); return; }
        _ => { render_message(f, inner, "Loading bracket…"); return; }
    };
...
fn render_series_body(f: &mut Frame, area: Rect, s: &PlayoffSeries) {
    ...
    let header = format!(
        "  {} ({})  vs  {} ({})",
        s.top_seed_name, s.top_seed_rank.as_deref().unwrap_or("—"),
        s.bottom_seed_name, s.bottom_seed_rank.as_deref().unwrap_or("—"),
    );
...
mod tests {
    ...
    fn fixture_series(letter: &str, top: &str, bot: &str, top_w: u8, bot_w: u8, conf: Option<&str>) -> PlayoffSeries {
        PlayoffSeries {
    ...
    fn l0_render_playoffs_idle_shows_loading() {
        ...
        assert!(text.contains("Loading"), "idle must show loading message, got:\n{text}");
    }
    ...
    fn l0_render_playoffs_off_season_when_empty() {
        ...
        seed(&mut app, vec![]);
        let text = render_to_text(&app);
        assert!(text.contains("Playoffs not yet active"),
    ...
    fn l0_render_playoffs_error_shows_retry_hint() {
        ...
        let text = render_to_text(&app);
        assert!(text.contains("unavailable"), "error path missing, got:\n{text}");
    ...
    fn l0_render_playoffs_loaded_shows_round_header_and_series() {
        let mut app = App::new(false);
        let r1 = PlayoffRound { round_number: 1, label: "First Round".to_owned(), series: vec![
        ...
        let r2 = PlayoffRound { round_number: 2, label: "Second Round".to_owned(),
        ...
        assert!(text.contains("EASTERN CONFERENCE"), "east header missing, got:\n{text}");
        ...
        assert!(text.contains("FLA wins"), "completed series summary missing, got:\n{text}");
    }
    ...
    fn l0_render_playoffs_active_round_changes_with_cursor() {
        ...
        assert!(text0.contains("Round 1 of 2"), "footer should show 'Round 1 of 2', got:\n{text0}");

        // Move to round 2
        app.playoffs_round = 1;
        let text1 = render_to_text(&app);
        assert!(text1.contains("Round 2 of 2"), "footer should show 'Round 2 of 2', got:\n{text1}");
    }
    ...
    fn l0_render_series_detail_shows_summary_and_games_played() {
        ...
        assert!(text.contains("FLA wins"), "summary must show FLA wins, got:\n{text}");
        assert!(text.contains("6 game(s) played"), "games_played count must appear, got:\n{text}");
    }
    ...
    fn l0_render_series_detail_unknown_letter_shows_message() {
        ...
        assert!(text.contains("Series Z not found"),
