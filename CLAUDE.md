...
- **Phase Foster — favorites dashboard, time-travel, unified data layer**: Five sub-phases shipped on top of a new data architecture (Foster.0).
   ...
   - **F.1 — Time axis**: `--date YYYY-MM-DD` on `tonight` and `schedule` (deprecated `--start` alias kept hidden); `Shift+D` opens a shared TUI date picker overlay on Tonight/Schedule (Playoffs reuses the season picker); web `/scores?date=` and `/schedule?date=` accept the same convention.
