---
wave: fire-the-kiln
pulse: icelines-signals-roster-empty-filter-copy
date: 2026-06-22
status: planned
req:
  - REQ-DOGFOOD-001
...
# Craft Chat Pulse icelines-signals-roster-empty-filter-copy

## Mission

WP-010 Signals: improve signals-roster --evidence no-match messaging so empty evidence filters report team size and filter scope without implying no skaters exist or creating ranking/cache/StatId claims.
...
- `craft dogfood pulse gate --workspace-root <ROOT> --session icelines-signals-roster-empty-filter-copy`
- `git diff --check`
