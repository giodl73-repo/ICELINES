---
wave: fire-the-kiln
pulse: icelines-signals-roster-evidence-filter
date: 2026-06-22
status: planned
req:
  - REQ-DOGFOOD-001
...
# Craft Chat Pulse icelines-signals-roster-evidence-filter

## Mission

WP-010 Signals: add a bounded evidence-tier filter to icelines signals-roster so users can inspect partial/missing evidence rows without creating a Signal leaderboard or promotion claim.
...
- `craft dogfood pulse gate --workspace-root <ROOT> --session icelines-signals-roster-evidence-filter`
- `git diff --check`
