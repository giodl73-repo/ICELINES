//! Official NHL completed contract-termination publication adapter.

use super::club_publication::extract_json_string_property;
use crate::adapter::{
    AbsenceSemantics, AdapterDisposition, AdapterError, AdapterErrorCategory, AdditiveFieldPolicy,
    HistoricalAvailability, SourceAdapter, SourceDescriptor, SourceInput,
};
use chrono::{DateTime, NaiveDate, TimeZone, Utc};
use icelines_core::source_facts::{
    AdapterId, AdapterVersion, EffectivePrecision, EffectiveTime, FactAuthority, FreshnessClass,
    OrganizationId, PlayerOrganizationEvent, ProposalId, ProviderId, ProviderIdentityProposal,
    ProviderPersonLocator, SourceEvidence, SourceFact, SourceId, SourceUrl, StagedAssertionId,
    StagedPlayerAssertion,
};

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct StagedPlayerRelease {
    pub proposal_id: ProposalId,
    pub organization: OrganizationId,
    pub occurred_at: EffectiveTime,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TerminationPublicationOutput {
    pub identity_proposal: ProviderIdentityProposal,
    pub release: StagedPlayerRelease,
    pub staged_assertion: StagedPlayerAssertion,
}

#[derive(Debug, Clone)]
pub struct NhlArticleContractTerminationAdapter {
    organization: OrganizationId,
    organization_display_name: String,
    captured_at: DateTime<Utc>,
    source_url: SourceUrl,
}

impl NhlArticleContractTerminationAdapter {
    pub fn new(
        organization: &str,
        organization_display_name: &str,
        captured_at: DateTime<Utc>,
        source_url: &str,
    ) -> Result<Self, String> {
        let organization = organization.trim().to_ascii_uppercase();
        if !(2..=4).contains(&organization.len())
            || !organization.bytes().all(|byte| byte.is_ascii_uppercase())
        {
            return Err("organization must be a 2-4 letter NHL abbreviation".to_owned());
        }
        if organization_display_name.trim().is_empty() {
            return Err("organization display name must not be empty".to_owned());
        }
        Ok(Self {
            organization: OrganizationId::try_new(organization)
                .map_err(|error| error.to_string())?,
            organization_display_name: organization_display_name.trim().to_owned(),
            captured_at,
            source_url: SourceUrl::try_new(source_url).map_err(|error| error.to_string())?,
        })
    }
}

impl SourceAdapter for NhlArticleContractTerminationAdapter {
    type Output = TerminationPublicationOutput;

    fn descriptor(&self) -> SourceDescriptor {
        SourceDescriptor {
            source_id: SourceId::try_new(format!(
                "nhl-contract-termination:{}",
                self.organization.as_str()
            ))
            .expect("validated organization produces a source id"),
            provider: ProviderId::try_new("official_nhl_publication")
                .expect("static provider id is valid"),
            adapter_id: AdapterId::try_new("nhl.article.contract_terminated")
                .expect("static adapter id is valid"),
            adapter_version: AdapterVersion::try_new("v1")
                .expect("static adapter version is valid"),
            payload_family: "official_nhl_contract_termination",
            supported_layouts: &["nhl_article_jsonld.completed_contract_termination.v1"],
            required_identity_keys: &["articleBody", "datePublished", "displayed_name"],
            additive_field_policy: AdditiveFieldPolicy::Reject,
            freshness_class: FreshnessClass::Transactional,
            historical_availability: HistoricalAvailability::ProviderArchive,
            absence_semantics: AbsenceSemantics::NotEvidence,
            output_fact_families: &["identity_proposal", "staged_player_release"],
        }
    }

    fn parse(&self, input: SourceInput<'_>) -> Result<Self::Output, AdapterError> {
        let descriptor = self.descriptor();
        let document = std::str::from_utf8(input.bytes()).map_err(|error| {
            adapter_error(
                &input,
                &descriptor,
                AdapterErrorCategory::UnsupportedLayout,
                format!("termination publication is not UTF-8: {error}"),
            )
        })?;
        let article = extract_json_string_property(document, "articleBody").map_err(|message| {
            adapter_error(
                &input,
                &descriptor,
                AdapterErrorCategory::UnsupportedLayout,
                message,
            )
        })?;
        let first_sentence = article
            .split_once('.')
            .map(|(sentence, _)| sentence)
            .unwrap_or(article.as_str())
            .trim();
        let (displayed_name, organization_and_date) = first_sentence
            .split_once(" had his contract terminated by the ")
            .or_else(|| first_sentence.split_once(" had her contract terminated by the "))
            .ok_or_else(|| {
                adapter_error(
                    &input,
                    &descriptor,
                    AdapterErrorCategory::UnsupportedLayout,
                    "article does not state a completed contract termination".to_owned(),
                )
            })?;
        let published_organization = organization_and_date
            .split_once(" on ")
            .map(|(organization, _)| organization)
            .unwrap_or(organization_and_date)
            .trim();
        if normalize_words(published_organization)
            != normalize_words(&self.organization_display_name)
        {
            return Err(adapter_error(
                &input,
                &descriptor,
                AdapterErrorCategory::SemanticValidation,
                format!(
                    "published organization `{published_organization}` does not match `{}`",
                    self.organization_display_name
                ),
            ));
        }
        let date = extract_json_string_property(document, "datePublished")
            .ok()
            .and_then(|value| value.get(..10).map(str::to_owned))
            .and_then(|value| NaiveDate::parse_from_str(&value, "%Y-%m-%d").ok())
            .ok_or_else(|| {
                adapter_error(
                    &input,
                    &descriptor,
                    AdapterErrorCategory::MalformedRecord,
                    "datePublished must begin with YYYY-MM-DD".to_owned(),
                )
            })?;
        let evidence = SourceEvidence::new(
            input.source_id().clone(),
            self.source_url.clone(),
            descriptor.provider.clone(),
            self.captured_at,
            input.content_hash().clone(),
            descriptor.adapter_version.clone(),
        );
        let proposal_id = ProposalId::try_new(format!(
            "contract-termination:{}:{}:{}",
            self.organization.as_str(),
            date.format("%Y-%m-%d"),
            normalize_key(displayed_name)
        ))
        .expect("validated termination fields produce a proposal id");
        let identity_proposal = ProviderIdentityProposal::new(
            proposal_id.clone(),
            ProviderPersonLocator::SourceRow {
                source_id: input.source_id().clone(),
                row_key: "completed-contract-termination:1".to_owned(),
            },
            displayed_name.trim(),
            None,
            None,
            vec![evidence.clone()],
        )
        .map_err(|error| {
            adapter_error(
                &input,
                &descriptor,
                AdapterErrorCategory::SemanticValidation,
                error.to_string(),
            )
        })?;
        let occurred_at = EffectiveTime::new(
            Utc.from_utc_datetime(&date.and_hms_opt(0, 0, 0).expect("midnight is a valid time")),
            None,
            EffectivePrecision::Day,
        )
        .expect("single-ended effective time is valid");
        let release = StagedPlayerRelease {
            proposal_id: proposal_id.clone(),
            organization: self.organization.clone(),
            occurred_at: occurred_at.clone(),
        };
        let staged_assertion = StagedPlayerAssertion::new(
            StagedAssertionId::try_new(format!("staged:{proposal_id}:release"))
                .expect("validated proposal id produces a staged assertion id"),
            format!("proposal:{proposal_id}:release"),
            proposal_id,
            occurred_at,
            FactAuthority::LegalControl,
            SourceFact::PlayerOrganization(PlayerOrganizationEvent::Released {
                by: self.organization.clone(),
            }),
            vec![evidence],
        )
        .map_err(|error| {
            adapter_error(
                &input,
                &descriptor,
                AdapterErrorCategory::SemanticValidation,
                error.to_string(),
            )
        })?;
        Ok(TerminationPublicationOutput {
            identity_proposal,
            release,
            staged_assertion,
        })
    }
}

fn adapter_error(
    input: &SourceInput<'_>,
    descriptor: &SourceDescriptor,
    category: AdapterErrorCategory,
    message: String,
) -> AdapterError {
    AdapterError {
        source_id: input.source_id().clone(),
        adapter_id: descriptor.adapter_id.clone(),
        input_hash: input.content_hash().clone(),
        category,
        disposition: AdapterDisposition::FatalSource,
        message,
    }
}

fn normalize_words(value: &str) -> String {
    value
        .split_whitespace()
        .map(str::to_ascii_lowercase)
        .collect::<Vec<_>>()
        .join(" ")
}

fn normalize_key(value: &str) -> String {
    value
        .chars()
        .map(|character| {
            if character.is_ascii_alphanumeric() {
                character.to_ascii_lowercase()
            } else {
                '-'
            }
        })
        .collect::<String>()
        .split('-')
        .filter(|part| !part.is_empty())
        .collect::<Vec<_>>()
        .join("-")
}
