...
async fn run_list() -> anyhow::Result<()> {
    ...
    for s in Scheme::all_builtins() {
        println!("{:<20} {:<12} {}", s.name, format!("{:?}", s.source).to_lowercase(), s.description);
    }
...
pub async fn run_show(name: &str) -> anyhow::Result<()> {
    ...
    let w = &scheme.skater;
    let fields: &[(&str, f32)] = &[
        ("goals", w.goals), ("assists", w.assists),
        ("pp_goals", w.pp_goals), ("pp_assists", w.pp_assists),
        ("sh_goals", w.sh_goals), ("sh_assists", w.sh_assists),
        ("gwg", w.gwg), ("ot_goals", w.ot_goals),
        ("hits", w.hits), ("blocks", w.blocks),
        ("shots_on_goal", w.shots_on_goal), ("plus_minus", w.plus_minus),
        ("takeaways", w.takeaways), ("giveaways", w.giveaways),
    ...
    let g = &scheme.goalie;
    let gfields: &[(&str, f32)] = &[
        ("wins", g.wins), ("losses", g.losses), ("saves", g.saves),
        ("goals_against", g.goals_against), ("shutouts", g.shutouts),
        ("save_pct", g.save_pct),
    ];
    for (k, v) in gfields {
        if v.abs() > 0.0001 {
...
async fn run_from_csv(path: &str, name: Option<&str>) -> anyhow::Result<()> {
    ...
    const STAT_COLS: &[(&str, &str)] = &[
        ...
        ("PPG (P)", "pp_goals"),("PPA (P)", "pp_assists"),
        ("SHG (P)", "sh_goals"),("SHA (P)", "sh_assists"),
        ("GWG (P)", "gwg"),     ("HIT (P)", "hits"),
        ("BLK (P)", "blocks"),  ("W (G)",   "goalie_wins"),
        ("L (G)",   "goalie_losses"), ("GA (G)", "goalie_ga"),
        ("SV (G)",  "goalie_saves"),  ("SHO (G)","goalie_shutouts"),
    ];

    let detected: Vec<&str> = STAT_COLS.iter()
