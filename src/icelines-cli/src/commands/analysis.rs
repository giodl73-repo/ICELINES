...
use icelines_core::{
    filter::PlayerFilter, model::Player, name::normalize_name,
...
pub async fn run_class(year: u16, pos: Option<String>, top: Option<usize>, _json: bool) -> anyhow::Result<()> {
    ...
    for p in matched.iter().take(limit) {
        ...
        println!("{:<5} {:<24} {:<5} {:<4} {:<5} {:<7} {:<8}",
            pick, p.full_name, p.team.as_str(), p.position.abbreviation(), age, ppg, proj);
    }
    Ok(())
}
...
pub async fn run_peers(player_name: String, size: usize, _json: bool) -> anyhow::Result<()> {
    ...
    filter.draft_years = Some(vec![
        draft_year.saturating_sub(1), draft_year, draft_year + 1
    ].into_iter().filter(|&y| y > 0).collect());
    ...
    let target_rank = peers.iter().position(|p| {
        p.name_normalized == target.name_normalized
    }).map(|i| i + 1).unwrap_or(0);

    println!("PEERS OF {} ({} · {:?} · Draft {})",
        target.full_name, target.team.as_str(), target.position, draft_year);
    println!("Peer group: Draft class {}-{} at {:?}", draft_year.saturating_sub(1), draft_year + 1, target.position);
    println!("{}", "─".repeat(64usize));
    println!("{:<5} {:<24} {:<5} {:<7} {:<8}", "Rank", "Player", "Team", "PPG", "Proj/82");
    println!("{}", "─".repeat(64usize));

    for (i, p) in peers.iter().take(size).enumerate() {
        let marker = if p.name_normalized == target.name_normalized { " ←" } else { "" };
        let (ppg, proj) = pace_strings(p);
        println!("{:<5} {:<24} {:<5} {:<7} {:<8}{}",
            i + 1, p.full_name, p.team.as_str(), ppg, proj, marker);
    }
    println!("\n{} in peer group. {} ranks #{} of {}.",
        target.full_name, target.full_name, target_rank, peers.len());
    Ok(())
}
...
pub async fn run_compare(name1: String, name2: String, _json: bool) -> anyhow::Result<()> {
    ...
    println!("{:<28} {:<28}", p1.full_name.as_str(), p2.full_name.as_str());
...
pub async fn run_history(player_name: String, _json: bool) -> anyhow::Result<()> {
    ...
    println!("CAREER HISTORY — {} ({} · {:?})", p.full_name, p.team.as_str(), p.position);
    println!("{}", "─".repeat(60usize));
    println!("{:<10} {:<6} {:<4} {:<4} {:<4} {:<7}", "Season", "Team", "GP", "G", "A", "Proj/82");
    println!("{}", "─".repeat(60usize));

    if let Some(s) = p.pace_score {
        let (_, proj) = pace_strings(p);
        println!("{:<10} {:<6} {:<4} {:<4} {:<4} {:<7}",
            "2025-26", p.team.as_str(), s.gp, p.season_goals, p.season_assists, proj);
    } else {
        println!("No pace-eligible season data available (GP < {}).", icelines_core::model::MIN_GP);
    }
...
pub async fn run_group(cmd: GroupSubcommand) -> anyhow::Result<()> {
    let groups_path = {
        ...
        std::path::PathBuf::from(home).join(".icelines").join("groups.json")
    };
    ...
    match cmd {
        ...
        GroupSubcommand::Add { group, player } => {
            let members = groups[&group]["members"].as_array_mut()
        ...
        GroupSubcommand::List => {
            ...
            if map.is_empty() { println!("No groups. Create one with `icelines group create`."); return Ok(()); }
        ...
        GroupSubcommand::Show { name } => {
            let g = &groups[&name];
            if g.is_null() { bail!("group '{name}' not found"); }
            let members = g["members"].as_array().map(|a| a.as_slice()).unwrap_or(&[]);
            println!("GROUP: {name}  ({} members)", members.len());
            if let Some(d) = g["description"].as_str() { if !d.is_empty() { println!("  {d}"); } }
            println!("{}", "─".repeat(56usize));
            ...
            for m in members {
                ...
                if let Some(p) = players.iter().find(|p| p.name_normalized == norm) {
                    let (ppg, proj) = pace_strings(p);
                    println!("  {:<24} {:<5} {:<4} {} / {}",
                        p.full_name, p.team.as_str(), p.position.abbreviation(), ppg, proj);
                } else {
        ...
        GroupSubcommand::Delete { name } => {
            if groups.as_object_mut().and_then(|m| m.remove(&name)).is_some() {
                save(&groups)?;
...
fn find_player<'a>(players: &'a [Player], name: &str) -> anyhow::Result<&'a Player> {
    ...
    players.iter().find(|p| p.name_normalized.contains(&norm))
