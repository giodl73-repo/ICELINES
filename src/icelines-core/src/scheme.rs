...
impl Default for SkaterWeights {
    fn default() -> Self {
        Self {
            goals: 0.0, assists: 0.0, pp_goals: 0.0, pp_assists: 0.0,
            sh_goals: 0.0, sh_assists: 0.0, gwg: 0.0, ot_goals: 0.0,
            hits: 0.0, blocks: 0.0, shots_on_goal: 0.0, plus_minus: 0.0,
            takeaways: 0.0, giveaways: 0.0, faceoff_wins: 0.0,
        }
    }
}
...
impl Default for GoalieWeights {
    fn default() -> Self {
        Self {
            wins: 0.0, losses: 0.0, saves: 0.0,
            goals_against: 0.0, shutouts: 0.0, save_pct: 0.0,
        }
    }
}
...
pub enum SchemeSource { Yahoo, Espn, Cbs, Custom }
...
impl Scheme {
    ...
    pub fn yahoo_standard() -> Self {
        Self {
            ...
            skater: SkaterWeights {
                ...
                pp_goals: 1.0, pp_assists: 0.5,
                sh_goals: 1.0, sh_assists: 0.5,
                gwg: 0.5, hits: 0.5, blocks: 0.5,
                ..Default::default()
            },
            goalie: GoalieWeights {
                wins: 5.0, losses: -2.0, saves: 0.15,
                goals_against: -1.0, shutouts: 4.0,
                ..Default::default()
            },
        }
    }
    ...
    pub fn espn_standard() -> Self {
        Self {
            ...
            skater: SkaterWeights {
                ...
                pp_goals: 2.0, pp_assists: 2.0,
                sh_goals: 3.0, sh_assists: 3.0,
                gwg: 1.0, hits: 1.0, blocks: 1.0,
                shots_on_goal: 1.0, plus_minus: 2.0,
                ..Default::default()
            },
            goalie: GoalieWeights {
                wins: 5.0, saves: 0.2, goals_against: -2.0, shutouts: 5.0,
                ..Default::default()
            },
        }
    }
    ...
    pub fn simple_pts() -> Self {
        Self {
            ...
            skater: SkaterWeights { goals: 1.0, assists: 1.0, ..Default::default() },
            goalie: GoalieWeights::default(),
        }
    }

    pub fn all_builtins() -> Vec<Self> {
        vec![Self::yahoo_standard(), Self::espn_standard(), Self::simple_pts()]
    }
}
...
pub fn compute_fantasy_score(
    ...
    let entries: &[(f32, &str)] = &[
        ...
        (weights.shots_on_goal * stats.shots_on_goal as f32, "shots_on_goal"),
        ...
        (weights.faceoff_wins * stats.faceoff_wins as f32, "faceoff_wins"),
    ];

    let total: f32 = entries.iter().map(|(v, _)| v).sum();
    let breakdown: HashMap<String, f32> = entries.iter()
...
mod tests {
    ...
    fn l0_scheme_breakdown_sums_to_total() {
        ...
        assert!(
            ...
            "breakdown sum {sum} != total {}", s.total
        );
    }
    ...
    fn l0_scheme_some_at_min_gp() {
        assert!(
            compute_fantasy_score(&beniers_stats(), &Scheme::yahoo_standard().skater, MIN_GP_SCHEME).is_some()
        );
    }
    ...
    fn l0_scheme_simple_pts_is_just_goals_assists() {
        ...
        let s = compute_fantasy_score(
            &beniers_stats(),
            &Scheme::simple_pts().skater,
            82
        ).unwrap();
        assert!((s.total - 50.0).abs() < 0.001, "expected 50.0, got {}", s.total);
    }
