...
use icelines_core::{
    ...
    TonightScoringIntelView, ViewContext,
};
...
pub fn load_player_scoring_profile(
    store: &DataStore,
    context: ViewContext,
    player_id: u32,
    ...
    let mut source_loaded = false;
    let mut events = Vec::new();
    for event in load_scoring_event_inputs(store).unwrap_or_default() {
        source_loaded = true;
        if event.shooting_player_id == Some(player_id) || event.scoring_player_id == Some(player_id)
        {
            events.push(event);
        }
    }
    ...
    PlayerScoringProfileView::from_source_events(
        context,
        player_id,
        player_name,
        source_loaded,
...
mod tests {
    ...
    #[test]
    fn l1_player_scoring_profile_filters_player_events() {
        let temp = tempfile::tempdir().expect("tempdir");
        let root = temp.path().join("data");
        let play_path = root
            .join("play_by_play")
        ...
        let view = load_player_scoring_profile(&store, context, 8483493, "Connor Bedard");

        assert_eq!(view.player_id, 8483493);
        assert_eq!(view.summary.goals, 1);
        assert_eq!(view.summary.shots_on_goal, 1);
        assert_eq!(view.events.len(), 1);
        assert_eq!(view.context.source_state[0].state, Completeness::Complete);
