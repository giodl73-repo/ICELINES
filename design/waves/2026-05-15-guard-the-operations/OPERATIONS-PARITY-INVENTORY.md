...
| Watch-rule arbitrary team/deployment editing | `surface-parity.md` watch rules row | done | 04 | Player-rule create/toggle/delete exists; richer rule dimensions stay deferred because `WatchRuleMutationIntent` has no validated team/deployment fields. TUI and web dashboard now reject unsupported team/deployment edit phrases instead of reinterpreting them as player rules. |
| Favorites/groups management | `surface-parity.md` favorites/groups row | done | 05 | Web can read any SQLite group through `?group=<name>` but mutates only canonical `Favorites`; dashboard rejects group create/delete/rename/member edits instead of GET-mutating; TUI `/fav add/remove` supports team abbrevs. |
