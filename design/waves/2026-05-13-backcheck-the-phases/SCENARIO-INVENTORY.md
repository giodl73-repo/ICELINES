...
| `future-product` | The scenario names behavior intentionally not shipped yet. | Sync `shifts` capability remains locked in `icelines-cli\tests\foster_capability_matrix.rs`; NHL EDGE skating-speed summaries remain blocked while goal-level Goal Visualizer tracking is separately supported; live destructive web data install/remove remains deferred unless a safe dry-run/local-only POST contract exists. | Keep as guardrail/deferred scope; do not force it into executable product tests before the feature exists. |
