...
| Focus model | **Implicit on middle / driven by command bar**. Side panes are nav widgets — Favorites Enter swaps middle; Schedule Enter opens that game/team in middle. No "Tab between panes" cycling. | User's choice: "i think 3 is the starting point [implicit focus] — maybe you have a 1 line command prompt at the bottom… so maybe its a bit of a chat-based CLI." Simpler model; matches the command-palette mental model. |
