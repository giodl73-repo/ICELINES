...
| `GET /favorites` | `handlers/favorites.rs` | HTML | Favorites/groups | partial - projects selected group membership through `FavoritesView`; `?group=<name>` is read-only and `Favorites` keeps POST-backed add/remove controls; covered by `l1_favorites_html_supports_read_only_group_selection` |
...
| `GET /api/v1/fantasy/roster-shape` | `handlers/fantasy.rs` | JSON API | Fantasy roster-shape validation | in progress; returns `RosterShapeValidationView` rows for persisted FantasyDb rosters and canonical player positions; no GET-backed roster-shape mutation |
