...
| Cross-team depth | `DepthLeagueView` | `depth` or equivalent | `tui depth` | `/depth` | `/api/v1/depth` | partial - web HTML, web JSON, and TUI league ranking build from `DepthLeagueView`; web JSON row identity is fenced by `l1_depth_json_rows_match_depth_league_view`; TUI first-row projection is fenced by `l1_tui_depth_league_render_matches_depth_league_view_first_row`; TUI depth Enter navigation and ranking render use `league_view_from_app`; CLI adapter alignment remains | Ted Lindsay |
...
| `GET /depth` | `handlers/depth.rs` | HTML | Cross-team depth | partial - projects depth rankings from `DepthLeagueView`; CLI and TUI team chart alignment remain |
