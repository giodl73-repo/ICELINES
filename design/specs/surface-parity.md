...
| Goalie leaderboard | `GoaliesView` | `query goalies` | `tui goalies` | `/goalies` | `/api/v1/goalies` | partial - CLI, TUI, web HTML, and web JSON build `GoaliesView`; CLI JSON/CSV row identity fenced by `l2_cmd_query_goalies_json_csv_row_identity_match`; web JSON supports CLI-parity `saves` sort and explicit `gp_min`; TUI saves adapter fenced by `l0_goalies_view_from_tui_state_carries_saves_sort_and_metric`; cross-surface row-order parity remains | Campbell/Ted Lindsay |
