...
| Cross-team depth | `DepthLeagueView` | `depth` or equivalent | `tui depth` | `/depth` | `/api/v1/depth` | partial - web HTML, web JSON, and TUI league ranking build from `DepthLeagueView`; web JSON row identity is fenced by `l1_depth_json_rows_match_depth_league_view`; TUI depth Enter navigation and ranking render use `league_view_from_app`; CLI adapter and TUI team chart alignment remain | Ted Lindsay |
