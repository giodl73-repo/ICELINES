...
| Analytics cache Web/API consumers | Named cache report plus scout, player-card, line, goalie, practice, postgame, adjustment, and agent routes remain WP-009 first-route Web/API evidence; Phase Penguins promotes only the coach dashboard to a bounded prepared-cache dashboard claim, not a finished coaching suite. | Keep each non-coach family partial until workflow evidence and product-copy review exist; coach remains bounded to prepared-cache read behavior without live recomputation or coaching authority. |
...
| Player signals (descriptive) | `PlayerSignalsView` | `signals <name>` (text + `--json` `signals.v1` envelope); `signals-roster --team <ABBR>`; `export md signals --player <name>` | player-card Signals block + `/player/:id/signals` handoff | `GET /player/:id/signals` | `GET /api/v1/player/:id/signals` | partial, Phase Hurricane / WP-010 plus Rangers/Capitals gates - CLI text, `--json` `signals.v1`, Markdown export, TUI player-card block, Web HTML, and Web JSON all render `PlayerSignalsView` (Physical Engagement Rate, Puck Management Differential, Penalty Drag Rate); `signals-roster` renders a team-scoped inspection matrix with `signals-roster.v1`; missing/partial evidence renders `unavailable`/`null` with evidence tier + missing inputs, never zero-fill; non-claim + disclosure copy travel with CLI/Web/export encodings; Phase Capitals keeps Signals out of `StatId`, leaderboards, analytics cache, and the `--filter` catalog until accepted cache metric keys, source-state/invalidation rules, and bounded catalog/leaderboard copy exist; L0 in `commands::signals`, `commands::export`, `player::hurricane_signals_tests`, and Web Signals handler; L1 Web route tests cover `GET /player/:id/signals` and `GET /api/v1/player/:id/signals`; L2 in `signals_system.rs` | Phase Hurricane/Capitals |
...
| Playoff bracket/detail | `PlayoffsView` | `playoffs` / `export md series` | `tui playoffs` / series detail | n/a | n/a | partial - TUI playoff detail renders bundled series game-log rows from `PlayoffsView` and adds a compact game-margin sparkline for nonzero margins, fenced by `l0_render_series_detail_with_games_shows_game_log`; Markdown `export md series` renders the same game-log boundary with an inline game-margin SVG | Campbell |
...
| Docs reference | `DocsView` | `docs` | in-TUI docs overlay | `/docs` | n/a | done - CLI docs, web `/docs`, and the TUI docs overlay use `DocsView`/embedded `COMMANDS.md`; the removed mkdocs/static-site CLI surface is not part of this row | Lester Patrick/Ted Lindsay/Jim Gregory |
...
| Docs/spec site | n/a | deferred static-site artifact | deferred - the mkdocs/static-site CLI surface and `/site/*` mount were removed; `icelines-site`, `docs/`, and `mkdocs.yml` remain on disk as supporting artifacts, not active user surfaces | Jim Gregory |
...
| `GET /reports/poach` | `handlers/poach.rs` | HTML report | Poach/weekly reports | done; appends inline poach-score SVG for positive report rows |
| `GET /reports/weekly` | `handlers/poach.rs` | HTML report | Poach/weekly reports | done; appends inline poach-score SVG for positive report rows |
