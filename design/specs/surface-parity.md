...
| Snapshot operations | `SnapshotView` / `MutationResultView` | `snapshot ...` | admin overlay | `/admin`, `/admin/snapshots/activate` | `/api/v1/admin/snapshots`, `/api/v1/admin/snapshots/activate`, `/api/v1/admin/snapshots/delete` | partial - `snapshot list` and `snapshot show` project through `SnapshotView`; CLI use/delete resolve through `SnapshotMutationIntent`; web HTML/JSON now render `SnapshotView`; web activate HTML forms and activate/delete JSON mutations resolve through `SnapshotMutationIntent` and return/derive `MutationResultView`; destructive HTML delete remains deferred | Jim Gregory/Ted Lindsay |
