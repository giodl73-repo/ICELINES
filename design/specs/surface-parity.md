...
| Feature | ViewModel | CLI | TUI | Web HTML | Web JSON | Status | Owner |
|---|---|---|---|---|---|---|---|
| Data install/list/remove | `DataStatusView` / `MutationResultView` | `data ...` | admin overlay | `/admin` | `/api/v1/admin/data-status` | partial - `data status` projects manifest rows through `DataStatusView`; CLI install/remove/verify resolve through `DataMutationIntent`; web HTML/JSON now render `DataStatusView`; web admin mutation routes remain deferred to Jim Gregory | Jim Gregory/Ted Lindsay |
| Snapshot operations | `SnapshotView` / `MutationResultView` | `snapshot ...` | admin overlay | `/admin` | `/api/v1/admin/snapshots` | partial - `snapshot list` and `snapshot show` project through `SnapshotView`; CLI use/delete resolve through `SnapshotMutationIntent`; web HTML/JSON now render `SnapshotView`; web admin mutation routes remain deferred | Jim Gregory/Ted Lindsay |
| Config/report toggles | `ConfigView` / `MutationResultView` | `config ...` | reports overlay | `/admin` | `/api/v1/admin/config` | partial - `config get/list` project through `ConfigView`; season-type and CLI config set/reset resolve through shared mutation intents; web HTML/JSON expose runtime web config through `ConfigView`; full CLI config/report-toggle mutation UI remains planned | Jim Gregory/Ted Lindsay |
