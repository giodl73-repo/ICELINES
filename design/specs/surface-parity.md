...
| Scoring reports | `GameScoringReportView` / `TeamScoringProfileView` | n/a | n/a | `/game/:id/scoring`, `/team/:abbrev/scoring` | `/api/v1/game/:id/scoring`, `/api/v1/team/:abbrev/scoring` | web/API Rocket Richard reports from cached official NHL play-by-play scoring events; summaries include goals, SOG, missed, blocked, attempts, unblocked attempts, period/situation splits, and top shooter IDs; empty states distinguish missing play-by-play from loaded zero-event payloads and team pages offer POST-backed cache loading | Rocket Richard/Ted Lindsay |
