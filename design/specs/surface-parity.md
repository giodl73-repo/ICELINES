...
| Compare/comps | `CompareView` / `SimilarPlayersView` | `query compare A B` / `query compare A --similar N` | `tui comps <name>` | `/compare?...` | `/api/v1/compare?...` | partial - web HTML and web JSON build `CompareView`; web JSON card identity is fenced by `l1_compare_json_cards_match_compare_view`; CLI head-to-head and TUI comps target card use `CompareView` card projection; CLI similarity and TUI comps list project from `SimilarPlayersView`; TUI target is fenced by `l1_tui_comps_target_matches_compare_view_anchor`; web similarity route remains | Ted Lindsay |
...
| `GET /compare` | `handlers/compare.rs` | HTML | Compare/comps | partial - projects compare page from `CompareView`; web similarity route remains |
