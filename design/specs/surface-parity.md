...
| Individual records | `PlayerRecordsView` / `TeamRecordsView` | `records player <name>`, `records team <ABBR>` | player/team hints plus cmdbar handoff | `/records/player/:id`, `/records/team/:abbrev` | `/api/v1/records/player/:id`, `/api/v1/records/team/:abbrev` | first slice done - player teams-scored-against and team players-scored-against-team project from persisted boxscore goal rows; TUI player/team screens expose records entry points and command-bar handoffs; goalie/fight metrics remain blocked on play-by-play participants | Ted Lindsay |
...
| `GET /records/player/:id` | `handlers/records.rs` | HTML | Individual records | projects `PlayerRecordsView` teams-scored-against rows from local persisted boxscores |
| `GET /records/team/:abbrev` | `handlers/records.rs` | HTML | Individual records | projects `TeamRecordsView` players-scored-against-team rows from local persisted boxscores |
| `GET /api/v1/records/player/:id` | `handlers/records.rs` | JSON | Individual records | returns `PlayerRecordsView` in the standard data/meta envelope |
| `GET /api/v1/records/team/:abbrev` | `handlers/records.rs` | JSON | Individual records | returns `TeamRecordsView` in the standard data/meta envelope |
