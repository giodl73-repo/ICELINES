...
|---|---|---|---|---|---|---|---|
| Leaders/skater leaderboard | `LeadersView` | `query leaders` | `tui stats` | `/leaders` | `/api/v1/leaders` | partial - CLI text, CLI JSON/CSV, web HTML, and web JSON build `LeadersView`; web JSON now applies repeated filters and discrete bio query filters; TUI stats still needs Ted/Lester adapter verification | Campbell/Ted Lindsay |
