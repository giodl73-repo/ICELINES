...
| Player signals (descriptive) | `PlayerSignalsView` | `signals <name>` (text + `--json` `signals.v1` envelope); `export md signals --player <name>` | player-card Signals block + `/player/:id/signals` handoff | `GET /player/:id/signals` | `GET /api/v1/player/:id/signals` | partial, Phase Hurricane / WP-010 pulse-05 - CLI text, `--json` `signals.v1`, Markdown export, TUI player-card block, Web HTML, and Web JSON all render `PlayerSignalsView` (Physical Engagement Rate, Puck Management Differential, Penalty Drag Rate); missing/partial evidence renders `unavailable`/`null` with evidence tier + missing inputs, never zero-fill; non-claim + disclosure copy travel with CLI/Web/export encodings; Signals stay out of `StatId`, leaderboards, analytics cache, and the `--filter` catalog until a later pulse; L0 in `commands::signals`, `commands::export`, `player::hurricane_signals_tests`, and Web Signals handler; L1 Web route tests cover `GET /player/:id/signals` and `GET /api/v1/player/:id/signals`; L2 in `signals_system.rs` | Phase Hurricane/PACE |
