...
| Scoring reports | `GameScoringReportView` / `TeamScoringProfileView` / `TonightScoringIntelView` | n/a | n/a | `/game/:id/scoring`, `/team/:abbrev/scoring`, `/tonight/intel` | `/api/v1/game/:id/scoring`, `/api/v1/team/:abbrev/scoring`, `/api/v1/tonight/intel` | web/API Rocket Richard reports from cached official NHL play-by-play scoring events; summaries include goals, SOG, missed, blocked, attempts, unblocked attempts, period/situation splits, top shooter IDs, and favorites-first daily scoring-intel rows; empty states distinguish missing play-by-play from loaded zero-event payloads and team/tonight pages offer POST-backed cache loading | Rocket Richard/Ted Lindsay |
