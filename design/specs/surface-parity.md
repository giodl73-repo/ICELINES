...
| Playoffs | `PlayoffsView` | `playoffs` | `tui playoffs` | `/playoffs` | `/api/v1/playoffs` | partial - CLI playoffs, TUI bracket list, TUI series-detail header/summary, web HTML, and web JSON project through `PlayoffsView`; `PlayoffsSeriesRow` carries stable letter, seed-rank, winner, and games-played fields for adapter output; TUI series-detail game log remains on richer per-game bracket data | Lester Patrick/Ted Lindsay |
...
| `GET /schedule` | `handlers/schedule.rs` | HTML | Schedule | partial - projects schedule rows from `ScheduleView`; CLI schedule and TUI week-list alignment landed; richer TUI team-season/matchup subviews remain |
