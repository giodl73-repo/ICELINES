...
| Cross-team depth | `DepthLeagueView` | `depth` or equivalent | `tui depth` | `/depth` | `/api/v1/depth` | partial - web HTML and web JSON build `DepthLeagueView`; web JSON row identity is fenced by `l1_depth_json_rows_match_depth_league_view`; TUI depth Enter navigation uses `DepthLeagueView` ordering via `league_view_from_app`; full TUI render adapter alignment remains | Ted Lindsay |
