...
| Team season performance | `TeamSeasonView` | `team-season <ABBR>` | `team <ABBR> season` | `/team/:abbrev/season` | `/api/v1/team/:abbrev/season` | partial - core, CLI text/JSON, TUI, web HTML, and web JSON project schedule-derived `TeamSeasonView`; distinct from roster/depth and raw schedule list surfaces; shipped metrics include record, points, home/away splits, one-goal split, recent form, remaining schedule counts, next opponents, and explicit standings/SOS source warning; standings context, playoff distance, strength of schedule, quality wins, and bad losses remain PT.3/PT.4 | Presidents Trophy |
