...
The lifecycle fields are implemented as a separate registry amendment rather
than additions to the current `organization_window_registry.v1` wire contract.
Its schema, reader compatibility, validator, author guide, and replay fixtures
land together. Any deprecated-method hold is sealed in that amendment against
the exact official manifest fingerprint; runtime-only exceptions are invalid.
...
The registry-lifecycle amendment is implemented as the separately sealed
`organization_window_registry_lifecycle.v1` policy over the immutable v1
inventory. It gates newly authored official Frames and custom rebases, binds
its fingerprint into new boards, and leaves saved-board replay unchanged.
`WindowProfileReadiness`, lifecycle, and observation status remain separate
axes. Deprecated-method holds are sealed, reviewed authorizations tied to the
exact official manifest fingerprint.
