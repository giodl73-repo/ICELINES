...
| W9 | in progress | Authoring/compatibility/cache documentation, additive compatibility and registered-profile extension fixtures, canonical loaded-board replay validation, three-OS fingerprint CI gate, strict affected-production lint, schema/golden checks, offline release smoke, Windows package verification, dependency audit, and measured performance baseline | Observe the three-OS gate in CI, run broader full-CI/no-network gates, and complete release closeout. |
