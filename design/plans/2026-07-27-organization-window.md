...
| W9 | in progress | Authoring/compatibility/cache documentation, additive compatibility and registered-profile extension fixtures, canonical loaded-board replay validation, observed three-OS fingerprint CI gate, strict affected-production lint, schema/golden checks, offline release smoke, Windows package verification, dependency audit, measured performance baseline, and live browser/accessibility review | Run broader full-CI/no-network gates, add the full package matrix and automated cross-surface golden parity, and complete release closeout. |
