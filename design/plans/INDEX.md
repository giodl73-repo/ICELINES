...
| [Phase Lindsay — Stat Catalog](2026-05-02-phaseLindsay-stat-catalog.md) | L.1+L.2+L.3 Implemented (v0.4) | Centralized `StatId` dispatch + 107 selectable stats + `--filter` grammar + categorized TUI Queries + sort picker + `fetch report --kind`. L.1 + L.2 + L.3 shipped 2026-05-02 (Tier-1 schema + StatId catalog + filter grammar + ExtraReports cache + `--filter` CLI + deterministic sort + categorized TUI sections + search-as-you-type sort picker; +141 Lindsay-prefixed tests, 1215 workspace tests passing). L.4-L.8 pending. |
