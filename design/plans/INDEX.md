...
| [Phase Lindsay — Stat Catalog](2026-05-02-phaseLindsay-stat-catalog.md) | L.1+L.2+L.3+L.4+L.5+L.5b Implemented (v0.4) | Centralized `StatId` dispatch + 108 selectable stats + `--filter` grammar + categorized TUI Queries + sort picker + career-table presets + catalog-keyed `--sort` / `--columns` / similarity / depth scoring / fantasy frozen-golden / axum roster JSON + site stat-name grep fence. L.1 + L.2 + L.3 + L.4 shipped 2026-05-02; L.5 + L.5b shipped 2026-05-03 (SortDispatch with legacy-first precedence, comps via StatId, `ScoringMode::Custom(StatId)`, `export md leaders --columns`, DI-25 frozen-golden for 3 built-in schemes, `/api/team/:name/roster` `stats` keyed by cli_key, WIRE-6 collision-pin test, `.gitattributes` for LF normalization, SI-03 site grep fence); +197 Lindsay-prefixed tests, 1271 workspace tests passing). L.6/L.7/L.8 pending. |
