...
| [Phase Lindsay — Stat Catalog](2026-05-02-phaseLindsay-stat-catalog.md) | L.1+L.2+L.3+L.4 Implemented (v0.4) | Centralized `StatId` dispatch + 108 selectable stats + `--filter` grammar + categorized TUI Queries + sort picker + career-table presets + `fetch report --kind`. L.1 + L.2 + L.3 + L.4 shipped 2026-05-02 (Tier-1 schema + StatId catalog + filter grammar + ExtraReports cache + `--filter` CLI + deterministic sort + categorized TUI sections + search-as-you-type sort picker + career-table column presets `[`/`]` + 80-col degradation; +177 Lindsay-prefixed tests, 1251 workspace tests passing). L.5-L.8 pending. |
