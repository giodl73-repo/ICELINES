...
| [Phase Lindsay — Stat Catalog](2026-05-02-phaseLindsay-stat-catalog.md) | L.1+L.2+L.3+L.4+L.5+L.5b+L.6 Implemented (v0.4) | Centralized `StatId` dispatch + 108 selectable stats + `--filter` grammar + categorized TUI Queries + sort picker + career-table presets + catalog-keyed `--sort` / `--columns` / similarity / depth scoring / fantasy frozen-golden / axum roster JSON + site stat-name grep fence + Tier-2 fetch. L.1 + L.2 + L.3 + L.4 shipped 2026-05-02; L.5 + L.5b + L.6 shipped 2026-05-03 (SortDispatch, comps via StatId, `ScoringMode::Custom(StatId)`, `export md leaders --columns`, DI-25 frozen-golden, axum `stats` map, WIRE-6 collision pin, `.gitattributes`, SI-03 site grep fence, Tier-2 generic fetch via url_path-derived filenames); +200 Lindsay-prefixed tests, 1274 workspace tests passing). L.7/L.8 pending. |
