...
- **Honest caveat on workspace-additive features** (King.1.x patch, forge review): cargo features are unioned across the workspace. Once `commands::serve` (King.1.5) wires `icelines-cli → icelines-web → icelines-core[send-sync]`, the CLI / TUI builds also see `send-sync = ON` for `icelines-core`, and the `assert_not_impl_any!(StatsRepository: Send, Sync)` lint silently disappears for them. Practically harmless — CLI/TUI never wrap the repo across threads, so they don't try to `Send` it — but the spec's claim that "CLI/TUI keep the safety net" is only true *until* King.1.5. After that, the lint is a tooling-time check tied to the feature being off, not a guaranteed property. To recover a build-time guard for CLI/TUI, a future change could split `icelines-core-web` (a wrapper crate that enables the feature) so `icelines-cli` doesn't pull it transitively. Tracked but not blocking.
