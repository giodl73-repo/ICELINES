...
4. **Verify CLI/TUI adapter alignment for partial rows**
   ...
   - Compare: web JSON card identity is now fenced against `CompareView` by
     ...
     yet in the `PlayerCardView` contract; CLI similarity, TUI comps list, and
     `/api/v1/compare?a=ID&similar=N` now project from `SimilarPlayersView`;
     web HTML similarity affordance remains before marking the row `done`.
