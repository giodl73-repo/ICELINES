...
- Team-season forecasts now retain trial-level 1–32 league-rank probabilities. `trade-pick-populate --season-forecast` uses the pick's original team to create a pre-lottery standings-order slot proxy, with an explicit uniform fallback for older artifacts and no claim to model lottery or playoff draft ordering.
