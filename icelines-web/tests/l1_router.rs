...
#[tokio::test]
async fn l1_docs_route_includes_watch_editor_boundary() {
    let app = router(WebState::new());
    let response = app
        .oneshot(
            Request::builder()
    ...
    let bytes = axum::body::to_bytes(response.into_body(), 512 * 1024)
        ...
        .expect("body fits");
    let body = std::str::from_utf8(&bytes).expect("docs html is utf-8");
    assert!(body.contains("web/TUI watch-rule editors"));
    assert!(body.contains("player-rule create"));
    assert!(body.contains("icelines watch deployment ... --save"));
    assert!(body.contains("team/deployment dimensions"));
}
...
async fn l1_dashboard_shell_renders_no_js_regions() {
    ...
    assert!(body.contains("Top leaders"));
    assert!(body.contains("jaw-preview-list"));
    ...
    assert!(body.contains("aria-label=\"Dashboard navigation\""));
    assert!(body.contains("<summary>Navigation</summary>"));
...
#[tokio::test]
async fn l1_dashboard_scores_workspace_embeds_full_scores_page() {
    let app = router(WebState::new());
    let response = app
        .oneshot(
            Request::builder()
                .uri("/dashboard?workspace=/scores?date=2024-10-04&partial=workspace")
                .body(Body::empty())
                .expect("request builder ok"),
        )
        .await
...
#[tokio::test]
async fn l1_dashboard_schedule_workspace_embeds_full_schedule_page() {
    let app = router(WebState::new());
    let response = app
        .oneshot(
            Request::builder()
                .uri("/dashboard?workspace=/schedule?team=EDM&partial=workspace")
                .body(Body::empty())
                .expect("request builder ok"),
        )
        .await
...
async fn l1_dashboard_shell_renders_pinned_pane_workspaces() {
    ...
    assert!(body.contains("aria-label=\"Open left pinned Player Card workspace in center\""));
    assert!(body.contains("aria-label=\"Swap left pinned Player Card workspace with center\""));
    assert!(body.contains("aria-label=\"Clear left pinned Player Card workspace\""));
    assert!(body.contains("aria-label=\"Open right pinned Team Depth workspace in center\""));
    assert!(body.contains("aria-label=\"Swap right pinned Team Depth workspace with center\""));
    assert!(body.contains("aria-label=\"Clear right pinned Team Depth workspace\""));
...
async fn l1_team_streaks_empty_state_loads_cache_in_web_ui() {
    ...
    assert!(
        body.contains(
            "Authority: cached per-game boxscore skater rows not loaded for team streak leaders"
        ),
        "body was:\n{body}"
    );
    assert!(
        body.contains(
            "Authority: cached official NHL play-by-play shot rows not loaded for team streak leaders"
        ),
        "body was:\n{body}"
    );
    assert!(
    ...
    assert!(authorities.iter().any(|authority| {
        authority["source_kind"] == "boxscore"
            ...
            && authority["covered_metrics"]
                == serde_json::json!([
                    "goal_streak_leaders",
                    "assist_streak_leaders",
                    "point_streak_leaders"
                ])
    }));
    assert!(authorities.iter().any(|authority| {
        authority["source_kind"] == "play_by_play"
            ...
            && authority["covered_metrics"]
                == serde_json::json!([
                    "shots_on_goal_streak_leaders",
                    "shot_attempt_streak_leaders"
                ])
    }));
}
...
#[tokio::test]
async fn l1_rocket_team_outlook_json_marks_unavailable_schedule_authority_when_missing() {
    let _guard = home_env_lock().await;
    let _data_root = DataRootEnvFixture::new();
    let app = router(WebState::with_repo(repo_with_mcdavid()));
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let json = response_json(response, 256 * 1024).await;
    assert_data_meta_envelope(&json, "team-outlook");
    assert_eq!(
        json["meta"]["schedule_authority"]["source"],
        serde_json::Value::from("NHL schedule and final scores")
    );
    assert_eq!(
        json["meta"]["schedule_authority"]["coverage_state"],
        serde_json::Value::from("unavailable")
    );
    assert_eq!(
        json["meta"]["schedule_authority"]["covered_metrics"],
        serde_json::json!([
            "remaining_games",
